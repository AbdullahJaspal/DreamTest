...
import {
  selectMyProfileData,
  selectShowHastagShareSheet,
  selectHastagShareContent,
...
const ShareHastag: React.FC<ShareSheetProps> = () => {
  const navigation = useNavigation();
  const dispatch = useDispatch();
  const my_data = useAppSelector(selectMyProfileData);
  const modalVisible = useAppSelector(selectShowHastagShareSheet);
  const video_details = useAppSelector(selectHastagShareContent);
  ...
  const closeModal = () => {
    dispatch(setHastagShareContent(''));
    setShare_user_id('');
    setSearched_user([]);
    dispatch(showHastagShareSheet(false));
  ...
  const toggleUserInfoVisibility = () => {
    ...
    setSearched_user(users); //Show User Information By Saurav
  };
