...
const ShareOtherReport: React.FC = () => {
  const {t, i18n} = useTranslation();
  const route = useRoute();
  const navigation = useNavigation();
  const my_data = useAppSelector((state: any) => state.my_data.my_profile_data);
  const [selectedImage, setSelectedImage] = useState(null);
  const [selectedVideo, setSelectedVideo] = useState(null);
  const [description, setDescription] = useState('');
  const [loading, setLoading] = useState(false);

  const {selected, idVideo} = route.params;
  ...
  const handleSendPress = async () => {
    try {
      ...
      formData.append('video_id', idVideo);
      ...
      const result = await videoApi.makeReport(formData, my_data?.auth_token);
...
export default ShareOtherReport;
