...
const SwitchToBalanceScreen: React.FC = () => {
  ...
  const handleExchangePressed = async () => {
    ...
    setLoading(true);
    try {
      // Fetch gift wallet diamonds and convert to wallet balance
      const response = await userApi.getGiftDiamondsIntoBalance(
        my_data?.auth_token,
      );

      if (response?.success) {
        const totalDiamonds = response.payload.total_diamonds;

        // Update wallet balance in the global state
        dispatch(update_wallet_diamond(totalDiamonds));

        // Show success alert and navigate to Balance screen
        Alert.alert(
          t('Success'),
          t('Your gift balance has been successfully converted into diamonds!'),
          [
            {
              text: 'OK',
              onPress: () => {
      ...
      console.error('Error exchanging diamonds:', error);
      Alert.alert(
        t('Error'),
        t(
  ...
  return (
    <View style={styles.main_container}>
      <Header headertext={t('Switch To Balance')} thirdButton={false} />
      ...
      <View style={[styles.nested_container, {marginTop: height * 0.02}]}>
        ...
        <View style={styles.currency_view}>
          <Text style={styles.balance_txt}>{diamonds()}</Text>
        </View>
      </View>
      ...
      <View style={styles.bottom_container}>
        ...
        <Pressable
          style={({pressed}) => [
            styles.exchange_button,
            {backgroundColor: pressed ? '#e8da69' : 'rgba(0, 0, 0, 0.1)'},
          ]}
...
export default React.memo(SwitchToBalanceScreen);
